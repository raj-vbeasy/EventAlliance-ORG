...
class EventsController extends ApiController
{
    ...
    private function _getUsersTeamsEvents($userId)
    {
        ...
        if (empty($checkSuperAdmin)) {
            ...
            $query                   = $tblEvents->find('all')->where(['start_date > ' => date("Y-m-d"), 'team_id IN' => $arrTeamId]);
    ...
    public function addEventTeamMember()
    {
        $requestData         = $this->request->getData();
        $tblEventTeamMembers = TableRegistry::get("EventTeamMembers");
        if ($requestData['status'] == 0) {
            $result = $tblEventTeamMembers->deleteAll(['event_id' => $requestData['event_id'],
                'user_id'                                             => $requestData['user_id'],
                'team_member_id'                                      => $requestData['team_member_id']]);
            ...
            $eventTeamMemberEntity->set('event_id', $requestData['event_id']);
            $eventTeamMemberEntity->set('team_member_id', $requestData['team_member_id']);
            $eventTeamMemberEntity->set('user_id', $requestData['user_id']);

            $tblEventTeamMembers->save($eventTeamMemberEntity);

            if (empty($eventTeamMemberEntity->errors())) {
                $return  = true;
                $id      = $eventTeamMemberEntity->id;
    ...
    public function getTeamMemberVote()
    {
       ...
       // echo "<prE>";print_r($rsTeamMembers);exit;
        unset($qurey);
